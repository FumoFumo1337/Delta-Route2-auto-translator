...
void Proxy::Init(HMODULE hProxy)
{
    ...
    UINT systemLength = GetSystemDirectoryW(realDllPath, static_cast<UINT>(std::size(realDllPath)));
    if (systemLength == 0 || systemLength >= std::size(realDllPath) - 12)
    {
        DeltaOverlay::LogDiagnostic(
            "[proxy] GetSystemDirectoryW failed or returned a truncated path: error " +
            std::to_string(GetLastError()));
        ExitProcess(ERROR_PATH_NOT_FOUND);
    }
    wcscat_s(realDllPath, L"\\winmm.dll");
    DeltaOverlay::LogDiagnostic("[proxy] loading real winmm.dll from " +
        WideToUtf8(realDllPath));
    OriginalModuleHandle = LoadLibraryW(realDllPath);
    if (OriginalModuleHandle == nullptr)
    {
        DeltaOverlay::LogDiagnostic("[proxy] LoadLibraryW(real winmm.dll) failed: error " +
            std::to_string(GetLastError()));
...
#define RESOLVE(fn) do { \
    Original##fn = GetProcAddress(OriginalModuleHandle, #fn); \
    if (Original##fn == nullptr) { \
        missingExports++; \
        DeltaOverlay::LogDiagnostic(std::string("[proxy] missing real winmm export ") + #fn + \
...
#undef RESOLVE
    if (missingExports != 0)
    {
        DeltaOverlay::LogDiagnostic("[proxy] refusing to start with " +
            std::to_string(missingExports) + " unresolved winmm exports");
        ExitProcess(ERROR_PROC_NOT_FOUND);
    }
    DeltaOverlay::LogDiagnostic("[proxy] real winmm.dll loaded; all forwarded exports resolved");
}
