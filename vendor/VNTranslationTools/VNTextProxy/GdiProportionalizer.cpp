...
{
    wstring str = DeltaOverlay::Decode(lpString, c);
...
{
    DeltaOverlay::LogDrawnText(pString, count);

    wstring text = DeltaOverlay::Decode(pString, count);
    ...
    x += DeltaOverlay::GlyphInkOffset(text.c_str(), text.size(), pFont->GetHeight());
